...
def extract_features(url):
    ...
    features['shortest_word_host'] = min((len(w) for w in re.split(r'[\W_]+', hostname) if w), default=0)
    features['shortest_word_path'] = min((len(w) for w in re.split(r'[\W_]+', path) if w), default=0)
